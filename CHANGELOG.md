...
### Elixir 1.20 / OTP 29 Compatibility

Makes the library compile cleanly under Elixir 1.20's set-theoretic type checker (OTP 29),
with no changes to the public API.

### Changed

- **Tool result handling extracted to `McpServer.Tool.CallResult.handle_tool_result/2`** — the
  validation logic previously inlined in the `call_tool/3` clauses generated by `McpServer.Router`
  now lives in a dedicated, pattern-matched function. Routing handler return values through its
  clauses (rather than a macro-inlined `case`) lets the type checker enforce the expected
  `{:ok, %CallResult{}}` / `{:ok, result}` / `{:error, reason}` return contract at each
  `use McpServer.Router` site — incompatible handler returns are reported at compile time (a hard
  error under `warnings_as_errors`), while runtime-only bad returns still raise and are mapped to
  `{:error, _}` as before.
- **Reordered `field/4` and `items/2` macro clauses** in `McpServer.Router` so the `do: block`
  clause precedes the `is_list(opts)` clause, avoiding a clause-ordering warning under the new
  compiler.
- **Removed an unreachable `valid_session_id?/1` catch-all clause** in `McpServer.HttpPlug`.

### Fixed

- Documentation fixes in `mix.exs`.
